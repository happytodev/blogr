...
it('includes language parameter in author links on series pages', function () {
    ...
    // Ensure no links without language prefix for authors
    $content = $response->getContent();
    preg_match_all('/href="([^"]*blog\/author\/[^"]*)"/', $content, $matches);
    
    foreach ($matches[1] as $link) {
        // Each author link should start with /en/ (current locale)
        expect($link)->toStartWith('/en/blog/author/');
    }
});
...
it('works with different locales', function () {
    ...
    foreach ($matches[1] as $link) {
        expect($link)->toStartWith('/fr/blog/author/');
    }
});
