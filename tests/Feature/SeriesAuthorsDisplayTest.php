...
it('series authors returns unique users from posts', function () {
    ...
    // Create 3 users
    $user1 = User::factory()->create(['slug' => 'author-one']);
    $user2 = User::factory()->create(['slug' => 'author-two']);
    $user3 = User::factory()->create(['slug' => 'author-three']);
...
it('series authors component shows plus indicator when exceeding limit', function () {
    ...
    for ($i = 1; $i <= 5; $i++) {
        $user = User::factory()->create(['slug' => "author-{$i}"]);
        BlogPost::factory()->create([
            'blog_series_id' => $series->id,
            'category_id' => $category->id,
            'user_id' => $user->id,
